...
from .tainting.decorators import taint_return
from .tainting.context_managers import TaintContext
from .tainting.logging import write_changes_to_csv

# Create your views here.
@login_required(login_url='signup')
#@taint_return(name="from_home")
def home(request):
...
class ShowProfilePageView(DetailView):
    ...
    def get_context_data(self,*args,**kwargs):
        ...
        with TaintContext() as ctx:
            context_profile_page = ctx.taint("context_profile_page", context)
        
        write_changes_to_csv() 
        return context
...
def like_post(request):
    ...
    if like_filter==None:
        ...
        with TaintContext() as ctx:
            tainted_num_likes = ctx.taint("num_of_likes", total_likes)
        post.save()
        write_changes_to_csv()
        return redirect('/')
...
def receive_js_tainted_data(request):
    try:
        ...
        file_name = 'js_tainted_variables.csv'
        ...
        new_headers = set([item['name'] for item in data]) - set(headers)
        ...
        for item in data:
            ...
            for change in item['changes']:
                ...
                existing_data[ts_str][name] = change['value']
